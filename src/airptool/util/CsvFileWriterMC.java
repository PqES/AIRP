...
public class CsvFileWriterMC {
	...
	public static void writeCsvFileMC(List<CsvDataMC> csvdataMC, String projectName, String projectPath) {
		...
		*/
		...
		try {
			fileWriter = new FileWriter(projectPath+"/"+projectName+"MC.csv", true);
			...
			for (CsvDataMC csvdMC : csvdataMC) {
				if(csvdMC.getId() == 0 && csvdMC.getId2() ==0){
					fileWriter.append("-");
					fileWriter.append(NEW_LINE_SEPARATOR);
				}
				else{
					...
					//double y = (double)(csvdMC.getA())/(double)(csvdMC.getA()+csvdMC.getB()+csvdMC.getC()+csvdMC.getD());
					
					//fileWriter.append(String.format("%.10f", y));
					
					fileWriter.append(csvdMC.getPacote());
					fileWriter.append(COMMA_DELIMITER);
					fileWriter.append(csvdMC.getClasse());
					fileWriter.append(COMMA_DELIMITER);
					fileWriter.append(csvdMC.getMetodo());
